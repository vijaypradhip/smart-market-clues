...
const SelectContext = createContext<{ value: string; onValueChange: (v: string) => void; open: boolean; setOpen: (v: boolean) => void }>({ value: '', onValueChange: () => {}, open: false, setOpen: () => {} })

export function Select({ value: controlledValue, defaultValue = '', onValueChange, children }: { value?: string; defaultValue?: string; onValueChange?: (v: string) => void; children?: ReactNode }) {
...
export function SelectContent({ className, children, ...props }: HTMLAttributes<HTMLDivElement>) {
  ...
  if (!open) return null
  return (<div ref={ref} className={cn('absolute z-50 mt-1 min-w-[8rem] overflow-hidden rounded-md border bg-popover p-1 text-popover-foreground shadow-md', className)} {...props}>{children}</div>)
}

export function SelectItem({ value, className, children, ...props }: { value: string } & HTMLAttributes<HTMLDivElement>) {
  const ctx = useContext(SelectContext)
  return (<div className={cn('relative flex cursor-pointer select-none items-center rounded-sm px-2 py-1.5 text-sm outline-none hover:bg-accent', ctx.value === value && 'bg-accent', className)} onClick={() => ctx.onValueChange(value)} {...props}>{children}</div>)
}