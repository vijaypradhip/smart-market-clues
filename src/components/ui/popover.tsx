...
export function Popover({ open: controlledOpen, defaultOpen = false, onOpenChange, children }: { open?: boolean; defaultOpen?: boolean; onOpenChange?: (v: boolean) => void; children?: ReactNode }) {
...
export function PopoverContent({ className, children, ...props }: HTMLAttributes<HTMLDivElement>) {
  ...
  if (!open) return null
  return (<div ref={ref} className={cn('absolute z-50 mt-1 w-72 rounded-md border bg-popover p-4 text-popover-foreground shadow-md outline-none', className)} {...props}>{children}</div>)
}